...
- **The server stores; the caller interprets.** Classes, scopes, and subjects are caller-defined strings, stored and returned verbatim. When you reach for a `const` that encodes what a class *means*, it belongs in the calling agent's judgment instead — or, if callers need the guidance at call time, in the tool description.
